/*
 * @(#)HTTPConnection.java				0.4 04/02/1998
 *
 ...
 * and <var>getInputStream()</var>). Following are some examples.
 *
 * <P>
...
public class HTTPConnection implements GlobalConstants, HTTPClientModuleConstants
	{
		...
		/** The tunnel to use (if any) */
		private static String Default_Tunnel_Host = null;
		...
		/** hack to disable Nagle */
		private static boolean disable_nagle = false;
		...
		static
			{
				...
				Class modules[] = { HTTPClient.RetryModule.class, HTTPClient.CookieModule.class,
						HTTPClient.RedirectionModule.class, HTTPClient.AuthorizationModule.class, HTTPClient.DefaultModule.class,
						HTTPClient.TransferEncodingModule.class, HTTPClient.ContentMD5Module.class,
		...
		*/
		 ...
		 *          the host
		 * @param port
		 *          the port, or -1 for the default port
		 ...
		 *
		 * @param host
		 *          the host to match - must be trim()'d and lowercase
		 ...
		 *          the absolute path of the file
		 * @param query
		 *          the query string; it will be urlencoded
		 * @return an HTTPResponse structure containing the response
		 * @exception java.io.IOException
		 *              when an exception is returned from the socket.
		 * @exception ModuleException
		 ...
		 * given then one is added with a value of
		 ...
		 * of bytes using the lower byte of each character. The request is sent
		 ...
		 *          the absolute path of the file
		 * @param stream
		 *          the output stream on which the data is written
		 * @return an HTTPResponse structure containing the response
		 * @exception java.io.IOException
		 *              when an exception is returned from the socket.
		 * @exception ModuleException
		 ...
		 * of bytes using the lower byte of each character. The request ist sent
		 * using the content-type "application/octet-stream".
		 *
		 * @see java.lang.String#getBytes(int, int, byte[], int)
		 * @param file
		 ...
		 *
		 * @param file
		 *          the absolute path of the resource, or "*"
		 * @return an HTTPResponse structure containing the response
		 * @exception java.io.IOException
		 *              when an exception is returned from the socket.
		 * @exception ModuleException
		 ...
		 *          the absolute path of the resource, or "*"
		 * @param headers
		 *          the headers containing optional info.
		 * @return an HTTPResponse structure containing the response
		 * @exception java.io.IOException
		 *              when an exception is returned from the socket.
		 * @exception ModuleException
		 ...
		 *          the headers containing optional info.
		 * @param data
		 *          any data to be sent in the optional body
		 * @return an HTTPResponse structure containing the response
		 * @exception java.io.IOException
		 *              when an exception is returned from the socket.
		 * @exception ModuleException
		 ...
		 *          the headers containing optional info.
		 * @param stream
		 *          an output stream for sending the optional body
		 * @return an HTTPResponse structure containing the response
		 * @exception java.io.IOException
		 *              when an exception is returned from the socket.
		 * @exception ModuleException
		 *              if an exception is encountered in any module.
		 */
		public HTTPResponse Options(String file, NVPair[] headers, HttpOutputStream stream)
		...
		/**
		 * Connects to the host/port specified in the constructor. If necessary,
		 * proxies (HTTP or SOCKS) will be tunneled by this call. If no proxy has to
		 * be used, this method connects directly to the specified server. A call of
		 * this method is independent from all other method calls of this object.
		 * Everytime this method is called, a new Socket is created.
		 *
		 * @return The Socket which is connected to the specified server/host
		 *         (tunneled through a proxy, if necessary).
		 * @exception java.io.IOException
		 *              when an exception is returned from the socket.
		 * @exception ModuleException
		 *              if an exception is encountered in any module.
		 ...
		 * @throws ThreadInterruptedIOException
				...
				synchronized (this)
					{
						if (Proxy_Host == null)
							{
								/* we can use a direct connection or only a SOCKS proxy is used -> that's easy */
								resultSocket = getSocket(Timeout);
							}
						else
							{
								/* we have to tunnel an HTTP proxy, ugh */
								/* don't interfere with other calls and use an own StreamDemultiplexor -> we restore
								 * the old one later -> no problem because of synchronization (there will be no other
								 * calls while we are working)
								 */
								StreamDemultiplexor oldMultiplexor = input_demux;
								input_demux = null;
								try
									{
										/* setup our request */
										HTTPResponse response = setupRequest("CONNECT", Host + ":" + Port, null, null, null);
										if (response.getStatusCode() == 200)
											{
												/* CONNECT successful -> get the Socket from the demultiplexor */
												resultSocket = input_demux.releaseSocket();
											}
										else
											{
												...
												String error = "HTTPClient: Connect: CONNECT was not successful. HTTP status: "
														+ response.getStatusCode() + " reason: " + reasonLine;
									...
									{
										/* make sure, that CONNECT compatibility resources on the demultiplexor are released
										 */
										if (input_demux != null)
											{
												input_demux.releaseHttpConnectResources();
											}
										/* restore the original demultiplexor */
										input_demux = oldMultiplexor;
									}
							}
					}
		 ...
		 * @param file
		 *          the absolute path of the resource, or null
		 * @param data
		 *          optional data, or null
		 * @param headers
		 *          optional headers, or null
		 * @return an HTTPResponse structure containing the response
		 * @exception java.io.IOException
		 *              when an exception is returned from the socket.
		 * @exception ModuleException
		 *              if an exception is encountered in any module.
		 */
		public HTTPResponse ExtensionMethod(String method, String file, byte[] data, NVPair[] headers)
				throws IOException, ModuleException
			{
		 ...
		 *          the absolute path of the resource, or null
		 * @param stream
		 *          optional output stream, or null
		 * @param headers
		 *          optional headers, or null
		 * @return an HTTPResponse structure containing the response
		 * @exception java.io.IOException
		...
		public void stop()
			{
				...
				for (StreamDemultiplexor demux = (StreamDemultiplexor) DemuxList
						.enumerate(); demux != null; demux = (StreamDemultiplexor) DemuxList.next())
					{
						demux.abort();
					}
			}
		 ...
		 * are two exceptions to this rule: "Content-length" and "Host" headers are
		 * always ignored; and when posting form-data any default "Content-type" is
		...
		public void setRawMode(boolean raw)
			{
				...
				String[] modules = { "HTTPClient.CookieModule", "HTTPClient.RedirectionModule",
						"HTTPClient.AuthorizationModule", "HTTPClient.DefaultModule", "HTTPClient.TransferEncodingModule",
		 ...
		 * HTTPResponse.xxxx()) then <code>stop()</code> <strong>should</strong> be
		 * invoked.
		 *
		 * <P>
		 ...
		 *
		 * <P>
		 * <em>Note:</em> The read timeout only works with JDK 1.1 or later. Using
		 * this method with JDK 1.0.2 or earlier will only influence the connection
		 * creation.
		 *
		 * <P>
		 * <em>Note:</em> Because the decompression streams used by the
		 * ContentEncodingModule and TransferEncodingModule cannot be restarted
		 * after an InterruptedIOException you must either disable these modules or
		 * invoke <code>stop()</code> after receiving the exception.
		 *
		 * Using this method with JDK 1.0.2 or earlier will only influence the
		 * connection creation.
		 *
		 * @param time
		 *          the time in milliseconds. A time of 0 means wait indefinitely.
		 * @see #stop()
		 */
		 ...
		 * property <var>HTTPClient.Modules</var>. This must contain a "|" separated
		 * list of classes in the order they're to be invoked. If this property is
		 ...
		 * @param module
		 *          the module's Class object
		 * @param pos
		 *          the position of this module in the list; if <var>pos</var> >= 0
		 *          then this is the absolute position in the list (0 is the first
		 *          position); if <var>pos</var> < 0 then this is the position
		 *          relative to the end of the list (-1 means the last element, -2
		 *          the second to last element, etc).
		 * @return true if module was successfully added; false if the module is
		 *         already in the list.
		 * @exception ArrayIndexOutOfBoundsException
		 *              if <var>pos</var> > list-size or if <var>pos</var> <
		 *              -(list-size).
		 * @exception ClassCastException
		 *              if <var>module</var> does not implement the
		 *              <var>HTTPClientModule</var> interface.
		 * @exception RuntimeException
		 *              if <var>module</var> cannot be instantiated.
		 */
		 ...
		 * same context (i.e. they keep multiple lists, one for each context). The
		 ...
		 *
		 * @param context
		 *          the new context; must be non-null
		 * @exception IllegalArgumentException
		 *              if <var>context</var> is null
		 * @exception RuntimeException
		 ...
		 *              if no auth handler is installed or the auth handler does not
		 *              handle the Digest scheme
		 ...
		 * <var>http.proxyPort</var>. Whether <var>http.proxyHost</var> is set or
		 * not determines whether a proxy server is used.
		 *
		 * <P>
		 ...
		 * Specify the same <var>host</var> and <var>port</var> as in this method.
		 ...
		 * @param host
		 *          the host on which the proxy server resides.
		 * @param port
		 ...
		 *
		 * @param host
		 *          a host name, domain name, IP-address or IP-subnet.
		 * @exception ParseException
		 *              if the length of the netmask does not match the length of
		 *              the IP-address
		 */
						...
						if (ip_addr.length != ip_mask.length)
							{
								throw new ParseException(
										"length of IP-address (" + ip_addr.length + ") != length of netmask (" + ip_mask.length + ")");
							}
					}
				else
					{
		 ...
		 * not be used. The syntax for <var>host</var> is specified in
		 ...
		 * @param host
		 *          the host on which the proxy server resides. The port used is the
		 *          default port 1080.
		 */
		 ...
		 *          the port the proxy server is listening on.
		 * @param version
		 *          the SOCKS version the server is running. Currently this must be
		 *          '4' or '5'.
		 * @exception SocksException
		 *              If <var>version</var> is not '4' or '5'.
		 */
		 ...
		 *              if an exception is encountered in any module.
		 */
		private HTTPResponse setupRequest(String method, String resource, NVPair[] headers, byte[] entity,
				HttpOutputStream stream) throws IOException, ModuleException
			{
				Request req = new Request(this, method, resource, mergedHeaders(headers), entity, stream);
		 ...
		 * defaults, which take precedence over built-in defaults.
		 *
		 * The following headers are removed if found: "Host" and "Content-length".
		 *
		 * @param spec
		 *          the headers specified in the call to the method
				...
				for (sidx = 0; sidx < spec_len; sidx++)
					{
						...
						if (s_name.equalsIgnoreCase("Content-length") || s_name.equalsIgnoreCase("Host"))
							{
								continue;
							}
				...
				for (int idx = 0; idx < ModuleList.size(); idx++)
					{
						...
						try
							...
							{
								throw new Error(
										"HTTPClient Internal Error: could not " + "create instance of " + mod.getName() + " -\n" + e);
							}
					}
		 ...
		 *          the Response
		 * @param usemodules
		 *          if false then skip module loop
		 * @exception IOException
		 *              if any module or sendRequest throws it
		 * @exception ModuleException
		 *              if any module throws it
		 */
		void handleRequest(Request req, HTTPResponse http_resp, Response resp, boolean usemodules)
				throws IOException, ModuleException
			{
		 ...
		 *          the request
		 * @param con_timeout
		 *          the timeout to use when establishing a socket connection; an
		 *          InterruptedIOException is thrown if the procedure times out.
		 * @return the response.
		 * @exception IOException
		 *              if thrown by the socket
		 * @exception InterruptedIOException
		 *              if the connection is not established within the specified
		 *              timeout
		 * @exception ModuleException
		 *              if any module throws it during the SSL- tunneling handshake
		 */
				...
				synchronized (this)
					{
						 ...
						 */
						if (req.getMethod().equals("POST") && prev_resp != null && input_demux != null)
							{
								if (DebugConn)
									{
										Util.logLine("Conn:  Stalling Request: " + req.getMethod() + " " + req.getRequestURI());
								...
								try
									{
										...
										if (input_demux == null || (sock = input_demux.getSocket()) == null)
											{
												...
												if (Protocol == HTTPS)
													{
														if (Default_Tunnel_Host == null && Tunnel_Host != null)
															{
												...
												if (input_demux != null)
													{
														/* we don't need the HTTP CONNECT compatibility mode from the old
														 * demultiplexor any more -> deactivate it and release compatibility
														 * resources
														 */
														if (input_demux.isHttpConnectCompatibilityModeUsed())
															{
																input_demux.releaseHttpConnectResources();
															}
													}
												...
												/* if it is a CONNECT call, use the compatibility mode
												 */
												input_demux = new StreamDemultiplexor(Protocol, sock, this, req.getMethod().equals("CONNECT"));
										...
										/* We want to stick (the first part of) the data and the
										 * headers into the same network packet. However, this is
										 * only possible if the data doesn't need to be delayed and
										 * we don't have to wait for a 100-Continue. Furthermore,
										 * as the data size increases so does the overhead of copying,
										 * but the relative gain in network performance decreases
										 * (we're only saving 1 of N packets); therefore we only do
										 * this for smallish (< 10K) data. (Unfortunately Java does
										 * not provide scatter/gather io calls).
										 * Actually, if Nagle is enabled then the combining is
										 * done by the TCP stack and we needn't do this ourselves;
										 * however, this still reduces the number of system calls.
										 */
										if (req.getData() != null && req.getData().length > 0 && req.getData().length < 10000
												&& req.delay_entity == 0
												&& !(ServProtVersKnown && ServerProtocolVersion >= HTTP_1_1 && wf_cont))
											{
										...
										else
											{
												...
												try
													{
														if (ServProtVersKnown && ServerProtocolVersion >= HTTP_1_1 && wf_cont)
															{
												...
												if (req.getData() != null && req.getData().length > 0)
													{
														if (req.delay_entity > 0)
															{
																// wait for something on the network; check
																// available() roughly every 100 ms
								...
								catch (IOException ioe)
									{
										...
										if (try_count == 0 || ioe instanceof UnknownHostException || ioe instanceof InterruptedIOException
						...
						if (((!KeepAliveUnknown && !DoesKeepAlive) || !keep_alive
								|| (KeepAliveReqMax != -1 && KeepAliveReqLeft-- == 0)) && !req.getMethod().equals("CONNECT"))
							{
						...
						// We don't pipeline POST's ...
								!IdempotentSequence.methodIsIdempotent(req.getMethod()) || req.dont_pipeline || // Retries disable pipelining too
								NeverPipeline)
		 ...
		 * actual destination. Uses a socket factory, if given.
		 *
		 * @param con_timeout
		 *          if not 0 then start a new thread to establish the the connection
		 *          and join(con_timeout) it. If the join() times out an
		 *          InteruptedIOException is thrown.
		 * @param socketFactory
		 *          socket factory to use, if given if null, the default socket
		 *          factory (of the JRE) is used
		 * @throws ThreadInterruptedIOException
		 *           if the current thread was interrupted (Rolf Wendolsky 06/06/17)
		 */
		private Socket getSocket(int con_timeout)
				throws IOException, ThreadInterruptedIOException
			{
				Socket sock = null;
				...
				if (Tunnel_Host != null)
					{
						actual_host = Tunnel_Host;
						actual_port = Tunnel_Port;
					}
		 ...
		 *          the socket
		 * @param req
		 *          the request initiating this connection
		 * @param timeout
		 *          the timeout
		...
		private Response enableSSLTunneling(Socket[] sock, Request req, int timeout) throws IOException, ModuleException
			{
				...
				hdrs.copyInto(h);
				Request connect = new Request(this, "CONNECT", Host + ":" + Port, h, null, null);
		 ...
		 * entity and the entity isn't being sent with the Transfer-Encoding
		 * "chunked".
		 ...
		 * If no connection header is specified and the proxy is not known to
		 * understand HTTP/1.1 or later then a "Proxy-Connection: keep-alive" header
		 * is generated.
		 ...
		 *              IOException, or if an error occurs during parsing of a
		 *              header
		 */
		private String[] assembleHeaders(Request req, ExtByteArrayOutputStream hdr_buf) throws IOException
			{
				...
				if (req.getMethod().equals("CONNECT"))
					{
						hdr_buf.write(req.getMethod(), " ", Host);
						hdr_buf.write(":", Integer.toString(Port), "");
						hdr_buf.write(" ", RequestProtocolVersion, "\r\n");
					}
				else if (Proxy_Host != null && Protocol != HTTPS && !file.equals("*"))
					{
						hdr_buf.write(req.getMethod(), " http://", Host);
						hdr_buf.write(":", Integer.toString(Port), file);
						hdr_buf.write(" ", RequestProtocolVersion, "\r\n");
					}
				else
					{
				...
				/** @author modified by Stefan Lieske 2005/02/14 */
				// if (Port != 80)
				if ((Port != 80) || req.getMethod().equals("CONNECT"))
					{
						hdr_buf.write("Host: ", Host, ":");
						hdr_buf.write(Integer.toString(Port), "\r\n");
					}
				else
				...
				int ct_idx = -1, ua_idx = -1, co_idx = -1, pc_idx = -1, ka_idx = -1, ex_idx = -1, te_idx = -1, tc_idx = -1,
						ug_idx = -1;
				...
				if (ua_idx != -1)
					{
						...
						hdr_buf.write(version, "\r\n");
					}
				else
					{
						hdr_buf.write("User-Agent: ", version, "\r\n");
					}
		 ...
		 * @exception IOException
		 *              if any exception is thrown by the response
		 */
		boolean handleFirstRequest(Request req, Response resp) throws IOException
			{
				 ...
				 */
				if ((Proxy_Host != null && Protocol != HTTPS) && resp.getHeader("Via") == null)
					{
						ServerProtocolVersion = HTTP_1_0;
					}
